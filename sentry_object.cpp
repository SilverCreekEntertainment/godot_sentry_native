...
// _on_before_send callback is called after a crash but before it is sent to sentry
sentry_value_t _on_before_send(sentry_value_t event, void *hint, void *closure) {
	
	// Flush Log file
	bool original_flush = ProjectSettings::get_singleton()->get_setting("application/run/flush_stdout_on_print");
...
Sentry::Sentry() {
	...
	String app_version = GLOBAL_DEF("third_party/sentry/app_version", String("1.0")); // version of your game to report to Sentry
	send_ip_address = GLOBAL_DEF("third_party/sentry/send_ip_address", false); // Turn on to include ip address of user in crash report
	String app_name = ProjectSettings::get_singleton()->get_setting("application/config/name");
	
	if(sentry_dsn == "")
	{
		if(!Engine::get_singleton()->is_editor_hint())
			print_line("Sentry not enabled. Set third_party/sentry/dsn in Project Settings to enable.");
		return;
	}
	...
	String sentry_db_path = OS::get_singleton()->get_user_data_dir() + "/.sentry-native"; // sentry needs a path it can write to
...
{
	sentry_remove_user(); // I don't know if it is important to remove previous user or not
